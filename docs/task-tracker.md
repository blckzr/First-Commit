...
- [x] **Onboarding** — about you, target position, placement, generating; one page per step, resumable via `onboarding_step`. API (`GET /career-paths`, `GET /onboarding`, `PUT /onboarding/about`, `PUT /onboarding/target`, `POST /onboarding/placement`) enforces the step order; `RequireOnboardingStep` mirrors it in the browser for the experience
  - [x] ~~Placement questions are not specified~~ — placement **rates, then checks**. Migration `0002` lets an assessment belong to a skill; 25 questions across HTML, CSS, JavaScript and Git; passing writes `module_completions` with `method = 'tested_out'`. Measured first: **0 of 13** modules could ever be skipped, because all are certificate-required
    - [ ] **Are all 13 core modules really certificate-required?** That is what made skipping impossible. If one is genuinely optional — Branching and merging is the candidate — placement could drop it outright. A curriculum judgement, not a code one
...
- [~] **Coding exercise (§5.11)** — content, API, worker pipeline and screen are built; **the sandbox is not**
  ...
  - [x] **The sandbox — Judge0, built.** `Judge0Runner` behind the existing `Runner` interface, a harness that compiles the test cases into one program, and `npm run judge0:check` to prove it on this machine. 30 tests, mutation-tested with six defects (two of which found real gaps: an unobserved out-of-range guard, and a marker check nothing exercised). Setup in `docker/judge0/README.md`
    - [x] **Judge0 cannot run on this machine, and that is now measured.** `isolate` needs cgroup v1; the WSL 2 VM is cgroup v2 unified and refuses a v1 mount even to a privileged container (`Invalid argument`). The `systemd.unified_cgroup_hierarchy=0` flag does nothing because `docker-desktop` has no systemd. [judge0#583](https://github.com/judge0/judge0/issues/583), open. `Judge0Runner` stays for a Linux host
